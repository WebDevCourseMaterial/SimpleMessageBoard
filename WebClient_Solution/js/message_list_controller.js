...
simplemb.MessageListController.prototype.handleMessagesResponse_ =
  ...
  var messageResponse = /** @type {simplemb.MessageResponse} */
      (xhr.getResponseJson());
...
simplemb.MessageListController.prototype.renderUiForMessages_ =
  ...
  for (var i = 0; i < 20; i++) {
    ...
    if (i < messages.length) {
      // Fill the message and make it visible.
      goog.style.showElement(messageEls[i], true);
      nameEls[i].innerHTML = messages[i]['author'];
      commentEls[i].innerHTML = messages[i]['comment'];
    } else {
      // Make the message hidden.
      goog.style.showElement(messageEls[i], false);
    }
  }
};
